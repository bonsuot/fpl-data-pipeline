...
--   • Select only useful columns (drop noise)
--   • Cast string numbers to proper FLOAT64/INT64 types
--   • Rename cryptic fields to human-readable names
--   • Add derived columns (e.g., price in £ instead of raw cost)
--   • Parse timestamps to proper TIMESTAMP type
--   • Map IDs to readable labels (position names, team names)
--   • Deduplicate where needed
-- =============================================================================

-- Create Silver dataset
CREATE SCHEMA IF NOT EXISTS `fantasy-premier-league-488117.fpl_silver`
OPTIONS (
    description = "FPL Silver Layer - Cleaned and standardized data"
);

-- -----------------------------------------------------------------------------
-- SILVER: PLAYERS
-- Cleaned player data with proper types and human-readable fields
-- -----------------------------------------------------------------------------
CREATE OR REPLACE TABLE `fantasy-premier-league-488117.fpl_silver.players` AS
SELECT
    -- Identity
    p.id                                                    AS player_id,
    p.first_name,
    p.second_name                                           AS last_name,
    p.web_name                                              AS display_name,
    CONCAT(p.first_name, ' ', p.second_name)                AS full_name,

    -- Team & Position (mapped to readable names)
    p.team                                                  AS team_id,
    t.name                                                  AS team_name,
    ...
    -- Price (API returns price in tenths of £, e.g., 100 = £10.0)
    ...
    -- ICT Index (cast from string to float)
    ...
    -- Expected Stats (xG, xA — cast from string to float)
...
FROM `fantasy-premier-league-488117.fpl_bronze.players` p
LEFT JOIN `fantasy-premier-league-488117.fpl_bronze.teams` t
    ON p.team = t.id;


-- -----------------------------------------------------------------------------
-- SILVER: TEAMS
-- Cleaned team data with strength ratings
-- -----------------------------------------------------------------------------
CREATE OR REPLACE TABLE `fantasy-premier-league-488117.fpl_silver.teams` AS
SELECT
    id                                                      AS team_id,
    code                                                    AS team_code,
    name                                                    AS team_name,
    ...
    -- Strength Ratings (FPL's internal ratings)
...
FROM `fantasy-premier-league-488117.fpl_bronze.teams`;


-- -----------------------------------------------------------------------------
-- SILVER: GAMEWEEKS
-- Cleaned gameweek summaries with proper timestamps
-- -----------------------------------------------------------------------------
CREATE OR REPLACE TABLE `fantasy-premier-league-488117.fpl_silver.gameweeks` AS
SELECT
    id                                                      AS gameweek_id,
    name                                                    AS gameweek_name,
    deadline_time                                           AS deadline,
    finished                                                AS is_finished,
    data_checked                                            AS is_data_final,

    -- Scoring
    average_entry_score                                     AS avg_score,
    highest_score                                           AS highest_score,
    ranked_count                                            AS total_managers,

    -- Popular picks (these are player IDs — will be resolved in Gold)
...
FROM `fantasy-premier-league-488117.fpl_bronze.gameweeks`;


-- -----------------------------------------------------------------------------
-- SILVER: FIXTURES
-- Cleaned fixture data with team names joined in
-- -----------------------------------------------------------------------------
CREATE OR REPLACE TABLE `fantasy-premier-league-488117.fpl_silver.fixtures` AS
SELECT
    f.id                                                    AS fixture_id,
    ...
    -- Match Status
    f.finished                                              AS is_finished,
    f.started                                               AS is_started,
    ...
    -- Total Goals
    CASE
        WHEN f.finished THEN f.team_h_score + f.team_a_score
        ELSE NULL
    END                                                     AS total_goals,
...
FROM `fantasy-premier-league-488117.fpl_bronze.fixtures` f
LEFT JOIN `fantasy-premier-league-488117.fpl_bronze.teams` th ON f.team_h = th.id
LEFT JOIN `fantasy-premier-league-488117.fpl_bronze.teams` ta ON f.team_a = ta.id;


-- -----------------------------------------------------------------------------
-- SILVER: GAMEWEEK LIVE (Player Performance per Gameweek)
-- Cleaned per-player-per-gameweek stats with proper types
-- -----------------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS `fantasy-premier-league-488117.fpl_silver.gameweek_live` (
        player_id INT64,
        gameweek INT64,
        minutes INT64,
        starts INT64,
...
MERGE INTO `fantasy-premier-league-488117.fpl_silver.gameweek_live` AS target
USING (
    SELECT
        gl.id                                                   AS player_id,
        gl.gameweek,
        ...
        CURRENT_TIMESTAMP()                                     AS loaded_at
    FROM `fantasy-premier-league-488117.fpl_bronze.gameweek_live` gl
) AS source
